...
const demos = [
  ...
  { name: 'Ivy', category: 'AGENTIC WORK', title: 'Manage and ship agentic work.', copy: 'A workspace for nontechnical teams.', href: null, action: 'Demo offline', note: 'Not available publicly', tone: 'ivy' },
...
export default function DemosPage() {
  return <div className={styles.hub}>
    ...
    <div className={styles.cards}>{demos.map((demo, i) => <article key={demo.name} className={styles.card} data-tone={demo.tone}><div className={styles.cardTop}><span>{String(i + 1).padStart(2, '0')}</span><span>{demo.category}</span></div><h2>{demo.name}</h2><h3>{demo.title}</h3><p>{demo.copy}</p><div className={styles.launch}><>{demo.href ? <Link href={demo.href}>{demo.action}<span aria-hidden>↗</span></Link> : <span className={styles.unavailable}>{demo.action}</span>}</><small>{demo.note}</small></div></article>)}</div>
