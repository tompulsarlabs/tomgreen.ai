...
The redesign turns that causal sequence into the site’s shared structure rather than adding a standalone visual demo. Warm paper holds the editorial record; green-black plates represent operating state; signal green marks location and movement. Proof gets denser and the interface gets quieter as the visitor moves from Home to Work to a case study.

## What shipped

- A new global visual and motion token system: warmer paper, green-black operating surfaces, signal/constraint/product accents, harder rules, larger consequence typography and a four-duration motion grammar.
- A persistent TOM/GREEN operating header with route coordinate, current/pending state, reading progress, 44px targets and explicit keyboard focus.
...
- A rebuilt Home opening with the direct proposition, verified proof line, two routes and a responsive HTML/CSS operating field with no animation loop.
...
- `npm run test:e2e` — 20/20 Playwright tests passed against `next start`.
...
- Focus matches hover on evidence surfaces; dark surfaces use signal-colour focus; navigation exposes `aria-current`; primary targets remain at least 44px.
